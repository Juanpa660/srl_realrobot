...
def Env(env_name, seed, max_episode_length=1000, action_repeat=1):
    env_list = ['Jackal-v0', 'Turtlebot-easy-v0', 'Turtlebot-medium-v0', 'Turtlebot-hard-v0', 
                'Doggo-v0', 'Safexp-PointGoal1-v0', 'Safexp-CarGoal1-v0']
    if not env_name in env_list:
        raise ValueError(f'Invalid environment name.\nSupport Env list: {env_list}') 
    if 'jackal' in env_name.lower() or 'doggo' in env_name.lower() or 'turtlebot' in env_name.lower():
