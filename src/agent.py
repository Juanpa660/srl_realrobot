...
class Agent:
    def __init__(self, args):
        ...
        if args.fine_tune_training:
            # Freeze the layers we don't want to update
            for param in self.policy.fc1.parameters():
                param.requires_grad = False
            for param in self.policy.fc2.parameters():
                param.requires_grad = False

            for param in self.value.fc1.parameters():
                param.requires_grad = False
            for param in self.value.fc2.parameters():
                param.requires_grad = False

            for param in self.cost_value.fc1.parameters():
                param.requires_grad = False
            for param in self.cost_value.fc2.parameters():
                param.requires_grad = False

            for param in self.cost_std_value.fc1.parameters():
                param.requires_grad = False
            for param in self.cost_std_value.fc2.parameters():
                param.requires_grad = False

            self.fine_tuning = True
            self.v_optimizer = torch.optim.Adam(self.value.fc3.parameters(), lr=self.lr)
            self.cost_v_optimizer = torch.optim.Adam(self.cost_value.fc3.parameters(), lr=self.lr)
            self.cost_std_v_optimizer = torch.optim.Adam(self.cost_std_value.fc3.parameters(), lr=self.lr)
        else:
    ...
    def train(self, trajs):
        ...
        cost_mean = np.mean(np.mean(costs)/(1.0 - self.discount_factor))
        ...
        # get gradient
        if self.fine_tuning:
            grad_g = flatGrad(objective, 
                              list(self.policy.fc_mean.parameters()) + list(self.policy.fc_log_std.parameters()),
                                retain_graph=True)
            grad_b = flatGrad(-cost_surrogate,
                              list(self.policy.fc_mean.parameters()) + list(self.policy.fc_log_std.parameters()),
                                retain_graph=True)
        else:
        ...
        beta = 1.0
        if self.fine_tuning:
            init_theta = torch.cat([t.view(-1) for t in list(self.policy.fc_mean.parameters()) + list(self.policy.fc_log_std.parameters())]).clone().detach()
        else:
    ...
    def applyParams(self, params):
        n = 0
        if self.fine_tuning:
            for p in list(self.policy.fc_mean.parameters()) + list(self.policy.fc_log_std.parameters()):
                numel = p.numel()
    ...
    def Hx(self, kl:torch.Tensor, x:torch.Tensor) -> torch.Tensor:
        ...
        '''
        if self.fine_tuning:
            flat_grad_kl = flatGrad(kl, list(self.policy.fc_mean.parameters()) + list(self.policy.fc_log_std.parameters()), create_graph=True)
            kl_x = torch.dot(flat_grad_kl, x)
            H_x = flatGrad(kl_x, list(self.policy.fc_mean.parameters()) + list(self.policy.fc_log_std.parameters()), retain_graph=True)
        else:
    ...
    def save(self, ep):
        ...
        torch.save({
            ...
            }, f"{self.checkpoint_dir}/model"+str(ep)+".pt")
        print('[save] success.')

    def load(self, ep):
        if not os.path.isdir(self.checkpoint_dir):
            os.makedirs(self.checkpoint_dir)
        checkpoint_file = f"{self.checkpoint_dir}/model"+str(ep)+".pt"
    ...
    def load_pretrained(self, model_name):
        ...
        checkpoint_file = f"{current_dir}/saved_models/"+str(model_name)+".pt"
        if os.path.isfile(checkpoint_file):
            checkpoint = torch.load(checkpoint_file)
            self.policy.load_state_dict(checkpoint['policy'])
            self.value.load_state_dict(checkpoint['value'])
